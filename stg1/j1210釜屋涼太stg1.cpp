/*
ザコ敵及びボスは円運動のような動きをするように改良した。
弾の出し方は、30度から150度の範囲でランダムに出るようにした。
ボスを1体倒すごとに自機の移動スピードが1ずつ遅くなるようにし、最大4体倒すとゲームを終了するようにした。
*/
...
#define LEVEL_UP_SCORE 1000 //敵1機で100点、1000なら10機でレベルアップ
...
int stage=0;//現在ステージ(ステージ0からスタート)
...
void ActionPlayer()
{
	...
	double s_speed = 8;
	double s_angle = OMEGA( -GetRand(150) );
	double range = 16;
	...
	if( CheckHitKey(KEY_INPUT_LEFT) )
		player.x -= speed-stage;
	if( CheckHitKey(KEY_INPUT_RIGHT) )
		player.x += speed-stage;
	if( CheckHitKey(KEY_INPUT_UP) )
		player.y -= speed-stage;
...
void MakeEnemy()
{
	...
	if( t % born1 == 0 && t % (born2 + 2500) < born2)
	{
		...
		enemy[i].speed = GetRand(4) + 2;//速度は2～8
...
void ActionEnemy()
{
	...
	for(i = 0; i < MAX_ENEMY; i++){
		...
		{
		case STOP://何もしない
			enemy[i].x += cos( OMEGA(t)/*enemy[i].angle*/ );
			enemy[i].y += sin( OMEGA(t)/*enemy[i].angle*/ );
			break;

		case STRAIGHT://決まった方向に直進

			enemy[i].x += enemy[i].speed * cos( OMEGA(4*t)/*enemy[i].angle*/ ) +1;
			enemy[i].y += enemy[i].speed * sin( OMEGA(4*t)/*enemy[i].angle*/ ) +1;
